    ...
    protected function doDefine(Form $form)
    {
        ...
        $this->group('search', function (Form $form) {
            ...
            $this->list('field')
                ...
                ->option(__('mass.sakura.field.title'), 'sakura.title')
                ->option(__('mass.sakura.field.alias'), 'sakura.alias');
         ...
         *
         * You can override filter actions in SakurasModel::configureFilters()
         */
        $this->group('filter', function (Form $form) {
            // State
            $this->list('sakura.state')
                ->label('State')
                ->addClass('has-select2')
                // Add empty option to support single deselect button
                ->option('', '')
                ->option(__('mass.sakura.filter.state.select'), '')
